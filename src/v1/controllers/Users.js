...
const { findOne, insert, loginUser, del, update, _changePhoto, _changePassword, getAll, _changeUserType } = require('../services/Users')
const { passwordToHash, generateAccessToken, generateRefreshToken, passwordHashCompare } = require('../scripts/utils/helper')
...
const create = async (req, res) => {
    insert({ ...req.body, hashedPassword: await passwordToHash(req.body.Password) })
        .then(({ rows }) => res.status(httpStatus.CREATED).send(rows[0]))
        .catch((e) => {
            if (e.constraint === 'unique_username') return res.status(httpStatus.BAD_REQUEST).send({ error: 'Username already exists' })
            if (e.constraint === 'unique_email') return res.status(httpStatus.BAD_REQUEST).send({ error: 'Email already exists' })
...
const login = async (req, res) => {
    try {
        ...
        if (!Username || !Password) {
            return res.status(httpStatus.BAD_REQUEST).send({ error: 'Invalid input data. Username and Password are required.' });
        }
        ...
        const hashedPassword = await passwordHashCompare(Password, rows[0].passwordhash);
        if (hashedPassword) {
            const user = { ...rows[0] };
            delete user.photo;
            delete user.passwordhash;
            ...
            return res.status(httpStatus.UNAUTHORIZED).send({ error: 'Incorrect password' });
        }
    } catch (error) {
        console.error(error);
...
const changePassword = (req, res) => {
    findOne(req.user.userid)
        .then(async ({ rows }) => {
            if (!rows[0]) return res.status(httpStatus.NOT_FOUND).send({ message: 'There is no such record.' })
            const user = { ...rows[0] }
            const hashedPasswordCompare = await passwordHashCompare(req.body.oldPassword, user.passwordhash)

            if (hashedPasswordCompare) {
                if (req.body.newPassword !== req.body.newPasswordAgain)
                    return res.status(httpStatus.BAD_REQUEST).send({ error: 'New password and new password must be the same again.' })

                const passwordhash = await passwordToHash(req.body.newPassword)

                try {
                    await _changePassword({ userid: req.user.userid, passwordhash })
                    return res.status(httpStatus.CREATED).send({ message: 'The password has been changed successfully.' })
                } catch (e) {
                    return res.status(httpStatus.INTERNAL_SERVER_ERROR).send({ error: e })
                }
            }

            res.status(httpStatus.BAD_REQUEST).send({ error: 'Old password is wrong.' })
        })
        .catch((e) => res.status(httpStatus.INTERNAL_SERVER_ERROR).send({ error: e }))
}
